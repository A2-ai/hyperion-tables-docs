...
description: "Creates rules for constructing the `variability` display column. Rules are evaluated with `case_when()`."
pagefind: true
---
...
<tr>
<td><code>...</code></td>
<td>Formula expressions like `fixed ~ "(Fixed)"` or `!is.na(cv) ~ sprintf("(CV = %s%%)", cv)`</td>
</tr>
</tbody>
</table>
</div>
