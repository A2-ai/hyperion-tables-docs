...
description: "Creates rules for assigning parameters to named sections in the output table. Rules are evaluated after name transformation, so you can match on the final displ"
pagefind: true
---
...
<tr>
<td><code>...</code></td>
<td>Formula expressions following [`dplyr::case_when()`](https://dplyr.tidyverse.org/reference/case-and-replace-when.html)<span style = {{ display: 'inline-block', verticalAlign: 'middle' }}><Icon name="external" /></span> syntax, `kind == "THETA" ~ "Structural Parameters"`</td>
</tr>
</tbody>
</table>
</div>
